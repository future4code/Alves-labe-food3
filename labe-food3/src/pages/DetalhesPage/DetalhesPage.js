...
export const DetalhesPage = (body, clear, setIsloading) => {
  ...
  const restaurantDetails = useRequestData(
    [],
    `${BASE_URL}/restaurants/${params.restaurantId}`
  );
  ...
  const getRestaurantesDetalhes = () => {
    setIsloading(true);
    axios
      .get(`${BASE_URL}/restaurants/${params.restaurantId}`, body, {
        headers: {
  ...
  const filterCategories =
    categories &&
    categories.filter((categoria, index) => {
      ...
      filterCategories.map((categorie) => {
        return (
          <div key={Math.random()}>
            ...
            {details &&
              details.products.map((produto) => {
                if (categorie === produto.category) {
                  return <div product={produto} key={produto.id} />;
                }
              })}
          </div>
        );
